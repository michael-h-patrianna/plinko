...
for (let i = 0; i < trajectory.length; i++) {
  ...
  if (point.pegHit) {
    ...
    console.log(`Peg hit #${pegHits} at frame ${i}: x=${point.x.toFixed(1)}, y=${point.y.toFixed(1)} (${timeSinceLastHit} frames since last hit)`);
  }
}
...
console.log(`Target slot ${params.selectedIndex}: x=[${targetSlotLeft.toFixed(1)}, ${targetSlotRight.toFixed(1)}]`);
