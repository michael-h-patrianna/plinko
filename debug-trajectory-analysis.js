...
for (let i = 0; i < Math.min(trajectory.length, 160); i += 20) {
  ...
  console.log(`Frame ${i.toString().padStart(3)}: y=${point.y.toFixed(1).padStart(6)}, x=${point.x.toFixed(1).padStart(6)}, pegHit=${point.pegHit}`);
}
...
let maxYJump = 0;
let teleportCount = 0;
for (let i = 1; i < trajectory.length - 20; i++) { // Exclude settle phase
...
console.log(`Max Y jump between frames: ${maxYJump.toFixed(1)}px ${maxYJump > 30 ? '❌ SUSPICIOUS' : '✓'}`);
console.log(`Teleport count (>50px jumps): ${teleportCount} ${teleportCount > 0 ? '❌ BROKEN' : '✓'}`);
...
console.log(`Total peg hits: ${pegHits} ${pegHits < 5 ? '❌ TOO FEW' : pegHits > 50 ? '❌ TOO MANY' : '✓'}`);
...
for (let i = 20; i < Math.min(trajectory.length - 20, 140); i += 40) {
  ...
  console.log(`Frame ${i.toString().padStart(3)}: speed=${speed.toFixed(1).padStart(6)}px/s, dy=${dy.toFixed(2).padStart(6)}`);
}
