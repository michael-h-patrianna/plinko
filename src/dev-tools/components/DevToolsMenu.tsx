      ...
      >
        ...
        >
          <path
            ...
            stroke={DEMO_STYLES.button.text}
            strokeWidth="2"
            strokeLinecap="round"
      ...
      <AnimatePresence>
        {isOpen && (
          <AnimatedDiv
            ...
            style={{
              background: DEMO_STYLES.menu.background,
              border: DEMO_STYLES.menu.border,
              minWidth: '320px',
            }}
          >
            {/* Header */}
            <div
              ...
              style={{
                background: DEMO_STYLES.header.background,
                borderColor: DEMO_STYLES.header.borderColor,
              }}
            >
              <h3
                className="font-semibold text-xs"
                style={{ color: DEMO_STYLES.header.textColor }}
              >
                Dev Tools
              </h3>
            </div>
            ...
            {/* Theme Section */}
            <div className="p-2 border-b" style={{ borderColor: DEMO_STYLES.section.borderColor }}>
              <div className="flex justify-between items-center mb-1.5">
                <label className="block text-xs font-medium" style={{ color: DEMO_STYLES.label.color }}>
                  Theme
                </label>
                <AnimatedButton
                  ...
                  style={{
                    background: DEMO_STYLES.button.elevated,
                    color: DEMO_STYLES.button.text,
                    border: `1px solid ${DEMO_STYLES.button.border}`,
                  }}
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.95 }}
                    ...
                    style={{
                      background:
                        themeName === availableTheme.name
                          ? DEMO_STYLES.buttonActive.background
                          : DEMO_STYLES.button.elevated,
                      color:
                        themeName === availableTheme.name
                          ? DEMO_STYLES.buttonActive.text
                          : DEMO_STYLES.button.text,
                      border: `1px solid ${
                        themeName === availableTheme.name
                          ? DEMO_STYLES.buttonActive.border
                          : DEMO_STYLES.button.border
                      }`,
                    }}
                    whileHover={{ scale: 1.05 }}
                    whileTap={{ scale: 0.95 }}
            ...
            <div className="p-2 border-b" style={{ borderColor: DEMO_STYLES.section.borderColor }}>
              <label className="block text-xs font-medium mb-1.5" style={{ color: DEMO_STYLES.label.color }}>
                Choice Mechanic
              </label>
            ...
            {!isMobile && (
              <div className="p-2">
                <label className="block text-xs font-medium mb-1.5" style={{ color: DEMO_STYLES.label.color }}>
                  Viewport {viewportDisabled && <span style={{ color: DEMO_STYLES.warning }}>(Locked)</span>}
                </label>
                <div className="grid grid-cols-2 gap-1.5">
                  {VIEWPORT_SIZES.map(({ width, label, colorKey }) => {
                    const isSelected = viewportWidth === width;
                    const color = VIEWPORT_COLORS[colorKey];
                    ...
                    return (
                      <button
                        ...
                        style={{
                          background: isSelected ? color : DEMO_STYLES.button.elevated,
                          color: isSelected ? DEMO_STYLES.buttonActive.text : DEMO_STYLES.label.color,
                          border: `1px solid ${
                            isSelected ? color : DEMO_STYLES.button.border
                          }`,
