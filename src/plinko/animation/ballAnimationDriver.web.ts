...
import { animationAdapter } from '@plinko/utils/platform/animation';
  ...
  schedule(update: (frame: number) => void, config: AnimationTimingConfig): () => void {
    ...
    const tick = (timestamp: number) => {
      ...
      if (currentFrameIndex < totalFrames - 1) {
        ...
        // CRITICAL FIX: Call onComplete when animation reaches final frame
        // This ensures immediate transition to 'landed' state without waiting for timeout
        // The timeout (line 105-107) remains as a safety fallback
        onComplete();
      }
    };
