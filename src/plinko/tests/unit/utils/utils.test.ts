...
} from '@plinko/utils/deviceDetection';
import { abbreviateNumber } from '@plinko/utils/formatNumber';
import { calculateBucketHeight, calculateBucketZoneY } from '@plinko/utils/slotDimensions';
import { deviceInfoAdapter } from '@plinko/utils/platform/deviceInfo';
...
describe('prizeUtils', () => {
  ...
  describe('normalizeProbabilities', () => {
    it('should normalize probabilities that sum to 2.0', () => {
      ...
      expect(normalized.reduce((sum: number, p: PrizeConfig) => sum + p.probability, 0)).toBeCloseTo(1.0);
    });
    ...
    it('should create new array and not modify original', () => {
      ...
      const originalProbs = prizes.map((p: PrizeConfig) => p.probability);
      const normalized = normalizeProbabilities(prizes);
