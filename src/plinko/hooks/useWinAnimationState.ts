...
export function useWinAnimationState(
  ...
  // Initialize with different value to ensure initial state is processed
  const prevBallStateRef = useRef<GameState | null>(null);
  ...
  useEffect(() => {
    ...
    // Trigger landing impact when ball lands (including initial mount with 'landed' state)
    ...
    // Reset animations when ball state changes away from landed (but not on initial mount)
