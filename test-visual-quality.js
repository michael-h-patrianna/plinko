...
async function testVisualQuality() {
  ...
  try {
    ...
    const screenshotsDir = path.join(__dirname, 'quality-test-screenshots');
    ...
    const pegOverflow = await page.evaluate(() => {
      ...
      pegs.forEach(peg => {
        ...
        const BORDER = 12;
        if (pegCenterX < boardRect.left + BORDER ||
    ...
    console.log(`   ${parseFloat(slidingPercent) < 10 ? '✅' : '❌'}: Physics feels natural (< 10% sliding)\n`);
    ...
    console.log(`   Pegs with animations: ${pegAnimations.animatedCount}/${pegAnimations.totalPegs}`);
