  ...
  for (let seedOffset = 0; seedOffset < 5; seedOffset++) {
    ...
    for (let i = 1; i < trajectory.length; i++) {
      ...
      if (totalMovement < 0.5) {
        ...
        if (consecutiveStuckFrames > 10) {
          // More than 10 frames stuck
          stuckEvents.push({
    ...
    if (maxStuckDuration > 30) {
      ...
      console.log(
        `   Max stuck duration: ${maxStuckDuration} frames (${(maxStuckDuration / 60).toFixed(2)}s)`
      );
      console.log(`   Stuck events: ${stuckEvents.length}`);
      stuckEvents.forEach((event) => {
        console.log(
          `     - Frame ${event.frame}: stuck for ${event.duration} frames at y=${event.y.toFixed(1)}px`
        );
      });
      ...
      console.log(
        `⚠️  Slot ${targetSlot}, Seed ${seed}: Brief pause (${maxStuckDuration} frames) - acceptable`
      );
    } else {
...
console.log(
  `   Success rate: ${(((totalTests - failedTests.length) / totalTests) * 100).toFixed(1)}%`
);
