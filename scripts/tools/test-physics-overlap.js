  ...
  try {
    ...
    for (const point of trajectory) {
      ...
      for (const peg of pegs) {
        ...
        if (overlap > 1) {
          // More than 1px overlap
          overlapViolations++;
          maxOverlap = Math.max(maxOverlap, overlap);
          console.log(
            `  WARNING: Frame ${point.frame} - Ball overlaps peg at (${peg.x}, ${peg.y}) by ${overlap.toFixed(2)}px`
          );
        }
      }
    }
...
for (const peg of pegs) {
  ...
  if (leftDistance < PHYSICS.BORDER_WIDTH) {
    wallViolations++;
    console.log(
      `Peg at (${peg.x}, ${peg.y}) too close to left wall: ${leftDistance}px < ${PHYSICS.BORDER_WIDTH}px`
    );
  }
  if (rightDistance < PHYSICS.BORDER_WIDTH) {
    wallViolations++;
    console.log(
      `Peg at (${peg.x}, ${peg.y}) too close to right wall: ${rightDistance}px < ${PHYSICS.BORDER_WIDTH}px`
    );
  }
}
