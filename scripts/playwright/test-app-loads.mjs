...
async function testAppLoads() {
  ...
  try {
    ...
    const textContent = await page.evaluate(() =>
      document.body.textContent?.trim().substring(0, 100)
    );
    ...
    const elements = await page.evaluate(() => {
      return {
        ...
        hasViewportSelector:
          document.querySelector('select[aria-label="Select viewport"]') !== null,
    ...
    console.log('\nScreenshot saved as screenshots/app-state.png');
