...
console.log('Peg hit frames:', pegHits.map(f => `Frame ${f.frame}: Row ${f.pegHitRow}, Col ${f.pegHitCol}`).slice(0, 10));
...
for (let i = 1; i < trajectory.length; i++) {
  ...
  for (const peg of pegs) {
    ...
    if (dist < 16 && !frame.pegHit) { // Ball+peg radius = 16
      tunneling++;
      if (tunneling <= 3) {
        console.log(`TUNNELING at frame ${i}: Ball at (${frame.x.toFixed(1)}, ${frame.y.toFixed(1)}) passes through peg at (${peg.x.toFixed(1)}, ${peg.y.toFixed(1)}), dist=${dist.toFixed(1)}`);
      }
    }
  }
}
